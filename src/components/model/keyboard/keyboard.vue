<template>
  <div class="m-keyboard" :class="{
    ...
    <div class="m-keyboard-row" v-for="row in keys">
      <div class="m-kb-item" v-for="key in row" v-if="!key.hide">
        <button class="m-key" v-if="key.name" @click.stop="command(key)" :key="key.name" :data-key="key.name" :disabled="disabledKeys.indexOf(key.name) > -1">
      ...
      switch (key.name) {
        ...
        default:
          ...
          if (key.name === 'space') {
            ...
            ch = this.isSymbol
              ? key.symbol
              : this.isCapsLock ? key.upper : key.default
          }
