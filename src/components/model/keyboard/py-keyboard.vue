<template>
  <div class="m-py-keyboard">
    ...
    <template v-if="langType === 'zh'">
      <div class="m-keyboard-candidate-pinyin">
        <span v-if="mathResult && mathResult.pinyin" class="m-input-pinyin">{{mathResult.pinyin}}</span>
      </div>
      <div class="m-keyboard-candidate-word">
        <span class="m-candidate-word" v-for="(word, index) in candidates" :key="index" @click="selectWord(index)">
          <span class="m-candidate-word-index">{{index + 1}}.</span>{{word}}</span>
        <div class="m-candidate-word-page">
          <button class="up" :disabled="pagination.currentPage <= 1" @click="handlePageUp">▲</button>
          <button class="down" @click="handlePageDown" :disabled="pagination.pageCount <= pagination.currentPage">▼</button>
        </div>
      </div>
    </template>
    <keyboard ref="keyboard" v-bind="$attrs" v-on="$listeners" @key="handleKey" @back="handleBack" @enter="handleEnter" @lang="handleSwitchLang" :lang="this.langType" :langHide="false"></keyboard>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    candidates() {
      ...
      var page = this.pagination.slice(
