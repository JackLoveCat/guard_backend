<template>
  <div class="cut-box" ref="cut">
    ...
    <div class="cut-control-bg">
      <!-- 裁剪区 -->
      <div class="cut-view" v-show="showView" :style="{
        width: viewWidth + 'px',
        height: viewHeight + 'px',
        left: viewLeft + 'px',
        top: viewTop + 'px'
      }">
        <img src="" class="cut-view-img" ref="view-img" :style="{
          left: -viewLeft-1 + 'px',
          top: -viewTop-1 + 'px',
      ...
      <!-- 控制按钮 -->
      <div class="cut-control" v-show="showView && showCtrl" @mousedown.stop.prevent="viewClick" :style="{
        width: viewWidth + 'px',
        height: viewHeight + 'px',
        left: viewLeft + 'px',
        top: viewTop + 'px'
...
export default {
  ...
  computed: {
    ...
    viewLeft() {
      ...
      return this.vl + view.width >= this.cutPos.width
        ? this.cutPos.width - view.width
        : this.vl < 0 ? 0 : this.vl
    },
    viewTop() {
      if (!this.view) return 0
      var view = this.viewPos
      return this.vt + view.height >= this.cutPos.height
        ? this.cutPos.height - view.height
        : this.vt < 0 ? 0 : this.vt
    }
  },
  watch: {
