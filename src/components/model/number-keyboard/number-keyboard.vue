<template>
  ...
  <div class="m-number-keyboard" :style="{
    width: size + 'px',
    height: size + 'px'
  }">
    <div class="m-nkb-item" v-for="key in keys" :style="[{
      width: (key.val === 0 ? size/2 : size/4) + 'px',
      height: (key.val === 'enter' ? size/2 : size/4) + 'px'
    }]" :class="{'m-key-enter': key.val === 'enter'}">
      <button class="m-key" :data-val="key.val" :data-key="key.name" @click="handleClick(key)" :disabled="disabledKeys.indexOf(key.name) > -1">
        {{key.name}}
      </button>
    </div>
  </div>
