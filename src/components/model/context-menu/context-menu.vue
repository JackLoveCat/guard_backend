...
.app-context-menu {
  ...
  &.right .context-menu {
    & .context-menu-item.submenu .context-menu {
      left: auto;
      right: 100%;
      box-shadow: -3px 1px 5px rgba(0, 0, 0, 0.29);
    }
  }
}
.context-menu {
...
<template>
  ...
  <div class="app-context-menu" :class="{right: nearRight}" :style="{left: left, top: top}" v-show="isShow">
    <ul class="context-menu" v-for="menu in contextMenu.menus">
      <template v-for="item in menu.menus">
        <li class="context-menu-item" :class="{disabled: item.disabled, submenu: typeof item.submenu === 'object'}" @click.stop="handleCommand(item.command)">
          <i aria-hidden="true" class="menu-icon" v-if="hasIcon" :class="[item.icon]"></i>
          <span class="menu-text">{{item.text}}</span>
          <span class="menu-tips-text" v-if="item.tipsText" :class="{right: item.tipsRight}">{{item.tipsText}}</span>
          <ul class="context-menu" v-if="typeof item.submenu === 'object'" @click.stop="" v-for="submenus in item.submenu">
            <li class="context-menu-item" v-for="subitem in submenus.menus" :class="{disabled: subitem.disabled}" @click.stop="handleCommand(subitem.command)">
              <i aria-hidden="true" class="menu-icon" v-if="hasIcon" :class="[subitem.icon]"></i>
              <span class="menu-text">{{subitem.text}}</span>
              <span class="menu-tips-text" v-if="subitem.tipsText" :class="{right: subitem.tipsRight}">{{subitem.tipsText}}</span>
            </li>
          </ul>
        </li>
      </template>
...
export default {
  ...
  methods: {
    ...
    handleCommand(cmd) {
      // console.log(cmd)
      this.contextMenu.commands &&
        this.contextMenu.commands[cmd] &&
        this.contextMenu.commands[cmd]()
      this.hide()
    }
  },
